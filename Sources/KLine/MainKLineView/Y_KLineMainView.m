...
/**
 最高值 label 位置
 */
@property (nonatomic, assign) CGPoint highestLabelPosition;
@property (nonatomic, copy) NSString *highestLabelText;

/**
 最低值 label 位置
 */
@property (nonatomic, assign) CGPoint lowestLabelPosition;
@property (nonatomic, copy) NSString *lowestLabelText;
...
@implementation Y_KLineMainView {
  CGFloat minAssert; // min real value in line model
  CGFloat maxAssert; // max real value in line model
  
  CGFloat maxY; // max y value in view poistion
  CGFloat minY; // min y value in view poistion
  
  CGFloat unitValue; // convert rate
}
...
- (void)drawRect:(CGRect)rect{
  ...
  if(self.MainViewType == Y_StockChartcenterViewTypeKline)
  {
    ...
    [self.needDrawKLinePositionModels enumerateObjectsUsingBlock:^(Y_KLinePositionModel * _Nonnull kLinePositionModel, NSUInteger idx, BOOL * _Nonnull stop) {
      ...
      kLine.dateFormatter = _dateFormatter;
    ...
    [self.needDrawKLinePositionModels enumerateObjectsUsingBlock:^(Y_KLinePositionModel * _Nonnull positionModel, NSUInteger idx, BOOL * _Nonnull stop) {
      ...
      NSDate *date = [NSDate dateWithTimeIntervalSince1970:[self.needDrawKLineModels[idx].Date doubleValue] / 1000.0 ];
      
      //            NSDate *date = [[self formatter] dateFromString:self.needDrawKLineModels[idx].Date];
  ...
  [_highestLabelText drawAtPoint:_highestLabelPosition withAttributes:@{NSFontAttributeName : [UIFont systemFontOfSize:11],NSForegroundColorAttributeName : [UIColor assistTextColor]}];
  [_lowestLabelText drawAtPoint:_lowestLabelPosition withAttributes:@{NSFontAttributeName : [UIFont systemFontOfSize:11],NSForegroundColorAttributeName : [UIColor assistTextColor]}];
...
- (CGFloat)getRealYWithOriginYPosition: (CGFloat)originY
{
  ...
  return maxAssert - unitValue * (originY - minY);
}
...
- (CGPoint)getExactPositionWithOriginPosition:(CGPoint)origin {
  NSInteger startIndex = (NSInteger)((origin.x - self.startXPosition) / ([Y_StockChartGlobalVariable kLineGap] + [Y_StockChartGlobalVariable kLineWidth]));
  ...
  for (NSInteger index = startIndex > 0 ? startIndex - 1 : 0; index < arrCount; ++index) {
    ...
    if(origin.x > minX && origin.x < maxX)
    {
      ...
      {
        Y_KLineModel *kLinemodel = self.needDrawKLineModels[index];
        
        [self.delegate kLineMainViewLongPressKLinePositionModel:self.needDrawKLinePositionModels[index] kLineModel:kLinemodel];
      }
      return CGPointMake(kLinePositionModel.HighPoint.x, origin.y);
    }
...
- (NSArray *)private_convertToKLinePositionModelWithKLineModels {
  ...
  minAssert = firstModel.Low.floatValue;  // min value in the line model
  maxAssert = firstModel.High.floatValue; // max value in the line model
  ...
  Y_KLineModel *lastModel = kLineModels.lastObject;
  double first = [firstModel.Date doubleValue] / 1000.0;
  double last = [lastModel.Date doubleValue] / 1000.0;
  
  if (last - first < 24 * 3600) {
    _dateFormatter = @"HH:mm";
  } else if (last - first < 30 * 24 * 3600) {
    _dateFormatter = @"MM-dd";
  } else {
    _dateFormatter = @"yyyy-MM-dd";
  }
  
  __block NSUInteger highestIndex = 0;
  __block NSUInteger lowestIndex = 0;
  
  __block CGFloat highestValue = firstModel.High.floatValue;
  __block CGFloat lowestValue = firstModel.Low.floatValue;
  ...
  [kLineModels enumerateObjectsUsingBlock:^(Y_KLineModel * _Nonnull kLineModel, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if(kLineModel.High.floatValue > maxAssert)
    {
      ...
      highestIndex = idx;
      highestValue = kLineModel.High.floatValue;
    }
    if(kLineModel.Low.floatValue < minAssert)
    {
      minAssert = kLineModel.Low.floatValue;
      lowestIndex = idx;
      lowestValue = kLineModel.Low.floatValue;
    }
  ...
  _highestLabelText = [NSString stringWithFormat:@"%.6f", highestValue];
  _lowestLabelText = [NSString stringWithFormat:@"%.6f", lowestValue];
  
  maxAssert *= 1.0001;
  minAssert *= 0.9991;
  
  
  minY = Y_StockChartKLineMainViewMinY;
  maxY = self.parentScrollView.frame.size.height * [Y_StockChartGlobalVariable kLineMainViewRadio] - 15;
  
  unitValue = (maxAssert - minAssert)/(maxY - minY);
  ...
  for (NSInteger idx = 0 ; idx < kLineModelsCount; ++idx)
  {
    ...
    if (idx == highestIndex) {
      _highestLabelPosition = CGPointMake(highPoint.x, highPoint.y - 10);
    }
    
    if (idx == lowestIndex) {
      _lowestLabelPosition = lowPoint;
    }
